...
class UnifiedAfipProvider(AfipTokenProviderPort):
    ...
    Replica exactamente el login() de coadelpa-project/auth_service.py:
    ...
    def _post_password(self, view_state_pwd: str, action_url: str, *, referer: str) -> tuple[str, str, str]:
        ...
        if (not magyp_action or not token_input or not token_input.get('value') or 
            not sign_input or not sign_input.get('value')):
            return "", "", ""
    ...
    def _portal_open_app(self) -> None:
        ...
        self.http.get(f"{PORTAL_CF_BASE}/portal/app/", headers={
    ...
    def _portal_get_authorization(self) -> tuple[str, str]:
        ...
        url = f"{PORTAL_CF_BASE}/portal/api/servicios/{self.cuit}/servicio/senasa_traapi/autorizacion"
    ...
    def get_token_sign(self) -> tuple[str, str]:
        ...
        try:
            ...
            magyp_action, token_afip, sign_afip = self._post_password(view_state_pwd, action_url_pwd, referer=action_url_cuit)
        ...
        if not service_info or service_info.get('servicio', {}).get('serviceName') != 'senasa_traapi':
