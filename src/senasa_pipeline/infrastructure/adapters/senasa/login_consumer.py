...
class SenasaLoginConsumer(SenasaLoginPort):
    ...
    def _select_afip_user_and_complete_follow_up(self, initial_html: str, login_url: str):
        ...
        for i in range(12):  # ampliamos a 12 intentos por OIDC
            self._log(f"Follow-up iteration {i+1}/12")
            self._log_response_details(current, f"Follow-up-iter-{i+1}")
            current_url = getattr(current, 'url', '')
            # Si estamos en el IdP, intentar manejar callback form_post OIDC
            if 'idp.senasa.gob.ar' in str(current_url):
                self._log(f"Follow-up-iter-{i+1} -> At OIDC IdP, trying to handle callback form")
                oidc_resp = self._handle_oidc_callback_if_present(current)
                if oidc_resp:
                    current = oidc_resp
                    self._log_response_details(current, "OIDC-callback-submit")
                    continue
                else:
                    self._log("OIDC IdP page without callback form; will break to probe")
                    break
    ...
    def _handle_oidc_callback_if_present(self, response):
        ...
        for form in soup.find_all('form'):
            ...
            # Keycloak puede responder con form auto-post a redirect_uri (Login.aspx)
            if method == 'post' and 'Login.aspx' in action:
                callback_form = form
                break
        if not callback_form:
            # Intentar detectar JavaScript auto-post (document.forms[0].submit())
            # y buscar inputs code/state en el DOM, aunque no haya action explícito
            maybe_code = soup.find('input', {'name': 'code'})
            maybe_state = soup.find('input', {'name': 'state'})
            if maybe_code and maybe_state:
                # fallback: postear a redirect_uri detectado en scripts/meta
                redirect_uri = self._guess_redirect_uri_from_html(response.text)
                if redirect_uri:
                    self._log(f"Guessed redirect_uri from HTML: {redirect_uri}")
                    payload = {
                        'code': maybe_code.get('value', ''),
                        'state': maybe_state.get('value', '')
                    }
                    headers = {
                        "Content-Type": "application/x-www-form-urlencoded",
                    ...
                    try:
                        cb_resp = self.http.post(redirect_uri, data=payload, headers=headers)
                        return cb_resp
                    except Exception as e:
                        self._log(f"Fallback OIDC submit failed: {e}")
    ...
    def _guess_redirect_uri_from_html(self, html: str) -> str | None:
        # Buscar redirect_uri en scripts o inputs ocultos
        m = re.search(r"redirect_uri=([^&'\"]+)", html)
        if m:
            try:
                uri = m.group(1)
                # Puede venir urlencoded; confiar en que es absoluta
