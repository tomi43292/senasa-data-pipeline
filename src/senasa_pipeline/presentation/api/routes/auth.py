...
def ensure_session() -> dict[str, str]:  # type: ignore[misc]
    ...
    try:
        ...
        if result.status in ("REFRESHED", "ALREADY_ACTIVE"):
            store.save(
                consumer.cookies or http.dump_cookies(), result.expires_at or SystemClock().now()
            )
            return {
    ...
    provider_jsf = JSFProvider(http=http, cuit=settings.afip_cuit, password=settings.afip_password)
    uc2 = EnsureSenasaSessionUseCase(
        store=store,
        provider=provider_jsf,
        consumer=consumer,
        clock=SystemClock(),
        ttl_hours=settings.session_ttl_hours,
    )
    ...
    if result2.status in ("REFRESHED", "ALREADY_ACTIVE"):
        store.save(
            consumer.cookies or http.dump_cookies(), result2.expires_at or SystemClock().now()
        )
    return {
        "status": result2.status,
        "expires_at": result2.expires_at.isoformat() if result2.expires_at else None,
